...
# explore firm size and establishment size --------------------------------
fsize <- readRDS(here::here("data", "susb", "firmsize_wide.rds"))
glimpse(fsize)
skim(fsize) # 12 counties; 21 industries; 4 variables (emp, nestabs, nfirms, wages); 2 measures (pct, value)
count(fsize, naics, naicsdesc)
count(fsize, variable) # emp, nestabs, nfirms, wages
count(fsize, measure)

wide <- fsize |> 
  filter(measure=="value") |> 
  select(-measure, -total) |> 
  pivot_longer(c(elt20:e500p), names_to = "size") |> 
  mutate(fsize=case_when(size=="elt20" ~ 10,
                         size=="e2099" ~ 60,
                         size=="e100400" ~ 300,
                         size=="e500p" ~ 700)) |> 
  pivot_wider(names_from = c(variable, fsize)) |> 
  mutate(fsize=emp / nfirms,
         estsize=emp / nestabs,
         estpayroll=wages * 1000 / nestabs,
         avgwage=wages * 1000 / emp,
         fsizeratio=fsize / estsize)

check <- wide |> 
  filter(naics=="10") |> 
  arrange(county, estsize) 

cor(wide |> select(-c(1:6)), use="complete.obs")

wide2 <- wide |> filter(size != "total")
glimpse(wide2)

mod1 <- lm(fsize ~ estsize + estpayroll, data=wide2)
summary(mod1)

mod1a <- lm(fsize ~ estsize + I(estsize^2) + estpayroll, data=wide2)
summary(mod1a)


mod2 <- lm(fsize ~ estsize + estpayroll + county, data=wide2)
summary(mod2)

mod3 <- lm(fsizeratio ~ estsize + estpayroll + naicsdesc, data=wide2)
summary(mod3)

mod4 <- lm(fsize ~ estsize + I(estsize^2) + I(estsize^3) + avgwage + I(avgwage^2) + estpayroll + naicsdesc + county, data=wide2)
summary(mod4)



mod1 <- lm(fsizeratio ~ estsize + estpayroll, data=wide)
summary(mod1)

mod1a <- lm(fsizeratio ~ estsize + I(estsize^2) + estpayroll, data=wide)
summary(mod1a)

mod1b <- lm(fsizeratio ~ estsize + I(estsize^2) + I(estsize^3) + estpayroll, data=wide)
summary(mod1b)

coefs <- coef(mod1b)
tibble(estsize=0:1000,
       term0=coefs[1],
       term1=coefs[2] * estsize,
       term2=coefs[3] * (estsize^2),
       term3=coefs[4] * (estsize^3),
       ratio=term0 + term1 + term2 + term3,
       firmsize=ratio * estsize) |> 
  ggplot(aes(estsize, firmsize)) +
  geom_line()
...
mod2 <- lm(fsizeratio ~ estsize + estpayroll + county, data=wide)
summary(mod2)

mod3 <- lm(fsizeratio ~ estsize + estpayroll + naicsdesc, data=wide)
summary(mod3)


inds <- count(wide2, naics, naicsdesc)
wide2 |> 
  mutate(nratio=nfirms / nestabs) |> 
  filter(naics %in% inds$naics[7:12]) |> 
  ggplot(aes(estsize, fsize, colour=county)) +
  geom_point(size=3) +
  geom_abline(slope=1) +
  facet_wrap(~naics+naicsdesc, scales = "free", ncol=2) 
