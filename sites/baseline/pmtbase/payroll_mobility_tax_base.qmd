...
tab_mtatotal |> 
  ...
  tab_header(
    ...
    subtitle = "Selected items. Does not reflect small employer exemption or noncompliance"
  ) |> 
  cols_label(
...
tab_mtacounty <- pmtbase |> 
  ...
  group_by(mtasub, fips, area) |> 
  summarise(across(c(earnings, pmtbase), sum), .groups="drop") |> 
  adorn_totals() |> 
  mutate(tax_earnings=.0034 * earnings,
         tax_pmtbase=.0034 * pmtbase,
...
tab_mtacounty |> 
  ...
  cols_label(
    earnings = "Earnings",
    pmtbase = "PMT base after adjustment",
  ...
  fmt_currency(
    columns=c(earnings, pmtbase, tax_earnings, tax_pmtbase),
...
tabdata <- pmtbase |> 
  select(year, pmtwages, pmtnonwage) |> 
  group_by(year) |> 
  summarise(across(c(pmtwages, pmtnonwage), sum), .groups="drop") |> 
  mutate(pmtbase=pmtwages + pmtnonwage,
         ewagetax=.0034*pmtwages,
         enonwagetax=.0034*pmtnonwage,
