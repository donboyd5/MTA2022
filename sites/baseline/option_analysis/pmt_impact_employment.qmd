...
mtacos <- xwalkny |> 
  filter(mta_county) |> 
  select(unifips, uniname, boroname) |> 
  mutate(mta=ifelse(is.na(boroname), uniname, boroname))
...
### Employment in the MTA, New York State, and MTA-neighboring counties
...
tab <- tabdata |>
  select(date, 
         emp_nys, emp_adjacent, emp_mta,
         pdiff_nys, pdiff_adjacent, pdiff_mta) |> 
  gt() |> 
  tab_header(
