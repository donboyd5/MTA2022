...
# REDO the pmt entirely
#  Then examine policy options
#    all current exemptions
#    exempt govts from increase
#    exempt govts from base
#    exempt self emp from increase
#    exempt self emp from base
#    eliminate PRDO Putname Rockland Dutchess Orange (everything, govt)
...
tax_rev <- pmtrev |> 
  ...
  summarise(across(c(pmtrev_tot, pmtrev_wage, pmtrev_nonwage),
                   ~ sum(.x, na.rm=TRUE)),
            .by=year) |> 
  filter(year %in% 2015:2021) # 2022 looks bad and needs investigation
tax_rev
...
# my pmtbase data are by county, but not by industry
