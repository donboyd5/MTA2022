# Links, Priorities, and Agendas

## Fast links

-   [PMT self-employment income](pmtbase/pmt_net_selfemp_income.qmd)

-   [CFT vs. the PMT](misc/cft_vs_pmt.qmd)

-   [One-house budget resolutions](misc/one_house_budget_resolutions.qmd)

-   [casinos](misc/casinos.qmd)

-   [MTOA budget](mtoa/mtoa_budget.qmd)

-   [Unemployment insurance tax](misc/uitax.qmd)

-   [PMT local governments](pmt/pmt_local_governments.qmd)

-   [Internet marketplace tax](primers/primer_internet_marketplace_tax.qmd)

-   [Section 18-b](primers/primer_section_18b.qmd)

-   [PMT self-employment payors](pmt/pmt_self_employment_tax.qmd)
...
-   What would it cost to exempt local governments from the PMT completely? How much higher would the rate have to be?

-   Is the NYC TA a local government or part of the state government, legally, and in the QCEW? (It is an affiliate of the MTA, unlike other entities, which are subsidiaries.)

    -   On additional investigation (after the question below), I suspect that TA employees are in the local government owner code.
        -   statewide there were 52,283 local employees in NAICS 4851 in 2021, about 31% of the U.S. total; statewide, no state employees were disclosed, and private was very small

        -   So I think they must be in NAICS 4851 local; I don't know why there are no local ownership NAICS 4851 data published at the county level
    -   Another question is, who is the employer and who remits the tax
        -   Does the MTA, as a state employer?

        -   Does the TA, as a City employer?
    -   I sent the following question to the QCEW folks:

> Hi, I'm trying to figure out (1) where NYC transit workers would be in QCEW, and (2) whether they would be listed as local owner or state owner.
>
> For #1, I suspected NAICS 4851, but no data come up (they are suppressed) via the multi-table search for the 2 available owner codes, private and local, for New York County and for Kings County NY, the two counties likely to have the largest \# of employees.
>
> For #2, it is tricky. The MTA (Metropolitan Transportation Authority), the umbrella organization, is a state public benefit corporation. But transit employees generally are employed by the Transit Authority, technically an affiliate of the MTA that appears to belong to NYC. I thought if I could find data in state and local owner codes, I could figure it out because the TA would have so many employees that it would be clear whether they are in state or local. But the state owner code does not come up in the search of NAICS 4851 for New York or Kings County, and data for the local owner are suppressed (even though there must be tens of thousands of transit workers).
>
> Can you provide advice on how to answer these questions? Many thanks.

-   PMT offset by county (???)

-   PMT exemptions, by county (estimates, %)?

-   Data for estimating firm-specific impacts?

    -   Sent SUSB data

-   How much of state budget increase in 2023-24 goes to NYC? How does that differ from 2022-23?

-   The state budget reserve increasing from 4% to 15% according to the governor. Track it down.

    -   Bottom line: Yes, reserves are being increased dramatically, from 8% in SFY 2022 to 16% in SFY 2023, and then maintained (albeit with outyear gaps).

    -   Primary (but not only) driver is the uncertainty around timing of PTET revenue and associated PIT credits.

    -   Determining whether reserves really need to be this large would take some digging.

    -   Some details below.

### Reserves

In the [2023-24 Executive Budget financial plan document](https://www.budget.ny.gov/pubs/archive/fy24/ex/fp/fy24fp-ex.pdf), DOB says, "In October 2021, the Governor committed to building the State's reserves to ensure that it could honor its current commitments through good and bad times. The extraordinary gains in tax receipts have been directed to accomplish that purpose. By the end of FY 2023, the State will have boosted its reserves by over \$20 billion since FY 2020. It will have also prepaid over \$10 billion in future debt service costs that were due in FY 2024 through FY 2027."

"...the impact of an economic slowdown is highly unpredictable. A second, new risk has been created by the Pass-Through Entity Tax (PTET) program, which has introduced a high degree of uncertainty in the level and timing of PIT tax collections."

![](images/image-1887337479.png)

![](images/image-1005187056.png)

![](images/image-234555020.png)

![](images/image-2131709841.png)

![](images/image-2146552345.png)

![](images/image-111378700.png)

## Agendas

<!-- (pmt/payroll_mobility_tax.qmd#incidence) -->
...
-   Example employers - impact of 0.34 to 0.50 % rate; QCEW; employers from OS docs

-   the pandemic corp franchise tax increase and sunset - was there a written or implied agreement to make sure rate drops?

-   casino revenues - what are the revenue provisions (license fee and ongoing?), how were they to be split (state/city 80/20?), are some earmarked for education, what items are in law, what items are political agreements

-   UI tax, when scheduled for increase, how does it work, where are the data?
...
-   Discuss priorities
    ...
    -   MTOA distribution and model

### December 16, 2022, Friday

-   [PMT option to raise \$700 million](pmt/pmt_increase_dec2022.qmd), with and without state offset for legal changes
...
-   Examine impact of PMT, starting with simplest approach (see [this](pmt/pmt_howlarge.qmd), [this](pmt/pmt_impact_employment.qmd), and [this](pmt/pmt_payroll_tax_rates.qmd))
...
-   TCJA and MTA taxes see [this](primers/primer_tcja_ptet.qmd)
...
-   For [hypothetical PMT increase from 0.34% to 0.50%](pmt/pmt_increase.qmd)
...
-   Ways to analyze [impact of PMT on the economy](pmt/pmt_impact_employment.qmd)
...
-   FYI - I added a discussion on the [incidence of payroll taxes](pmt/payroll_mobility_tax.qmd#incidence)
